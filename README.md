# 《算法导论》编程练习

## [第二章：算法基础](Chapter_2)
### 2.1 插入排序
- [2.1.1 插入排序](Chapter_2/211_Insertion_Sort.py)
- [2.1.2 降序插入排序](Chapter_2/212_Revered_Insertion_Sort.py)
- [2.1.3 线性查找](Chapter_2/213_Linear_Search.py)
- [2.1.4 加两个二进制的数字](Chapter_2/214_Add_Two_Binary_Number.py)
### 2.2 分析算法
- [2.2.1 大0时间](Chapter_2/221_Big_O_Notation.md)
- [2.2.2 选择排序](Chapter_2/222_Choose_Sort.py)
### 2.3 设计算法
- [2.3.1 快速排序 Merge Sort](Chapter_2/231_Merge_Sort.py)
- [2.3.2 递归的插入排序](Chapter_2/232_Recursive_Insertion_Sort.py)
- [2.3.3 二叉搜索](Chapter_2/233_Binary_Search.py)
- [2.3.4 利用二叉搜索加速插入排序](Chapter_2/234_Insertion_Sort_With_Binary_Search.py)
- [2.3.5 Two Sum 问题](Chapter_2/235_Two_Sum.py)

## [第四章 分治算法](Chapter_4)
### 4.1 最大子数组问题
- [4.1.1 最大连续数有最大和 ( O(nlogn) 解法)](Chapter_4/412_Maximum_Continuous_Sum_V2.py) 
- [4.1.2 最大连续数有最大和 ( O(n) 解法)](Chapter_4/411_Maximum_Continuous_Sum_V1.py)
### 4.2 矩阵乘法
- [4.2.1 Strassen 矩阵乘法 O（nlg7）解法](Chapter_4/421_Strassen_Matrix_Multiplication.py)
- [4.2.1 Strassen 矩阵乘法 O（nlg7）解法详解](Chapter_4/421_Strassen_矩阵乘法_O（nlg7）解法详解.md)

## [第五章 概率分析和随机算法](Chapter_5)
### 5.1 雇佣问题
- [5.1.1 雇佣问题的伪代码](Chapter_5/511_Hire_Assistant_Problem.md)

## [第六章 堆排序](Chapter_6)
### 6.1 堆
- [6.1.1 堆数据结构的简介](Chapter_6/611_Heap.md)
- [6.1.1 堆的建议实现](Chapter_6/611_Heap.py)
